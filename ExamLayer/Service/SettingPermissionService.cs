...
{
    public class SettingPermissionService : ISettingPermissionService
    {
        private readonly IMapper _mapper;
        private readonly IGenericRepository<AZDPS_APIM_INDEX_LIST> _repository;
        public SettingPermissionService(IMapper mapper, IGenericRepository<AZDPS_APIM_INDEX_LIST> repository)
        {
            _mapper = mapper;
        ...
        public async Task<PagingSearchOutput<IndexBusinessMetaDto>> GetAllAsync(IndexBusinessMetaGetAllInput input)
        {
            try
            {
                ...
                var data_dto = _mapper.Map<List<IndexBusinessMetaDto>>(data).ToList();
